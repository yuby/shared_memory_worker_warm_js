...
self.onmessage = function(event) {
  ...
  switch (type) {
    case 'init':
      ...
      console.log('워커 2가 공유 버퍼로 초기화되었습니다');
      console.log('워커 2 범위:', workRange.start, '에서', workRange.end, '까지');
...
function processData() {
  ...
  for (let i = 0; i < sharedView.length; i++) {
    const currentValue = Atomics.load(sharedView, i);
    
    Atomics.store(sharedView, i, currentValue * 2);
  ...
  self.postMessage({ 
    type: 'processed',
    result: '워커 2에서 데이터 처리 완료 - 각 바이트 값 2배 증가'
  });
}
